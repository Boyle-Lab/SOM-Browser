...
#	    print STDERR "$chn\n";
	...
	if ($cnd eq "LIKE") {
	    ...
	    if ($base_table eq "neurons" && $tbl =~ m/genes/ && ($cnd eq "=" || $cnd eq "!=" || $cnd eq "LIKE")) {
		if ($cnd eq "=") {
		    ...
		    $qry .= ' EXISTS (SELECT pg.targetGene FROM peaks p INNER JOIN peaks_genes pg ON pg.id_peaks = p.id_peaks INNER JOIN neurons n ON n.id_neurons = p.id_neurons WHERE p.id_neurons = neurons.id_neurons AND pg.targetGene LIKE "%' . $val . '")';
		} else {
	    ...
	    } elsif ($tbl =~ m/factors/ && ($cnd eq "=" || $cnd eq "!=" || $cnd eq "LIKE")) {
		if ($base_table eq "peaks") {
		    if ($cnd eq "=") {
			...
			$qry .= ' EXISTS (SELECT f.name FROM factors f INNER JOIN peaks_factors pf ON pf.id_factors = f.id_factors WHERE pf.id_peaks = peaks.id_peaks AND pf.score = 1 AND f.name LIKE "%' . $val . '")';
		    } else {
			...
			$qry .= ' EXISTS (SELECT f.name FROM factors f INNER JOIN neurons_factors nf ON nf.id_factors = f.id_factors WHERE nf.id_neurons = neurons.id_neurons AND f.name LIKE "%' . $val . '")';
		    } else {
