...
#	    print STDERR "$chn\n";
	...
	if ($cnd eq "LIKE") {
		...
		if ($base_table eq "peaks") {
			...
			$qry .= ' EXISTS (SELECT 1 FROM factors f INNER JOIN peaks_factors pf ON pf.id_factors = f.id_factors WHERE pf.id_peaks = peaks.id_peaks AND pf.score = 1 AND f.name LIKE "%' . $val . '") ddd';
		    } else {
