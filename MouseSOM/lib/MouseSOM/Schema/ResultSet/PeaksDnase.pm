package MouseSOM::Schema::ResultSet::PeaksDnase;
...
sub get_peaks {
	...
	if ($show == 0) {
		...
		{ join => {'peaks_genes' => 'peaks' },
		  select => ['peaks.species', 'peaks.cell', 'peaks.chrom',
			     ...
			     'me.cov_m', 'me.cov_c', 'me.cov_k', 'me.cov_g'],
		  as => ['species', 'cell', 'chrom', 'chromstart', 'chromend',
			 'targetgenedist', 'cov_m', 'cov_c', 'cov_k', 'cov_g']
		}
                );
    ...
    my @header_a = ("Source", "N Peaks", "Cov_G", "Cov_K", "Cov_C", "Cov_M");
    my @header_b = ("Cell", "Location", "TssDist",  "Cov_G", "Cov_K", "Cov_C",
		    "Cov_M");
	...
	my $tmp = $peak->get_column('cov_g');
	if (!defined($tmp)) {
	    $row[3] = "NA";
	} else {
	    $row[3] = sprintf "%.4f", $peak->get_column('cov_g');
	}
	$tmp = $peak->get_column('cov_k');
	if (!defined($tmp)) {
            $row[4] = "NA";
	} else {
	    $row[4] = sprintf "%.4f", $peak->get_column('cov_k');
	}
	$tmp = $peak->get_column('cov_c');
	if (!defined($tmp)) {
            $row[5] = "NA";
        } else {
	    $row[5] = sprintf "%.4f", $peak->get_column('cov_c');
	}
	$tmp = $peak->get_column('cov_m');
	if (!defined($tmp)) {
            $row[6] = "NA";
        } else {
	    $row[6] = sprintf "%.4f", $peak->get_column('cov_m');
	}

	for (my $k = 3, my $l = 0; $l < 4; $k++, $l++) {
	    if ($row[$k] eq 'NA') {
		next;
	    }
	    if ($row[$k] > $global_max) {
